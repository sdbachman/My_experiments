...
for k in range(nz-2,-1,-1):
  U[0:-1,0:-1,k] = U[0:-1,0:-1,k+1]+(dh[k]*g/rho/f0) * numpy.diff(dens[:,:,k],1,1)/numpy.diff(YU[0:-1,:,k+1],1,1)
  U[:,-1,k] = 0      # Get endpoint
  U[-1,:,k] = U[0,:,k]            # In the wall
...
theta_pert = numpy.zeros((ny,nz))
theta_mean = numpy.mean(theta, 0)
theta_z_mean = numpy.mean(theta_mean, 0)
for i in range(0, numpy.alen(theta[0,0,:])):
  theta_pert[:,i] = theta_mean[:,i] - theta_z_mean[i]
...
thetap = numpy.transpose(theta,(2,1,0))
U2p = numpy.transpose(U2,(2,1,0))
...
w_nc_fid = Dataset('Zgrid_diagnostics.nc', 'w', format='NETCDF4')
#w_nc_fid.description = "Initial temperature field" %\
#                      (nc_fid.variables['theta'].var_desc.lower(),\
#                       theta_desc['name'], nc_fid.description)
...
# Using our previous dimension info, we can create the new time dimension
# Even though we know the size, we are going to set the size to unknown
w_nc_fid.createDimension('zw', 11)
w_nc_fid.createDimension('zt', 10)
w_nc_dim = w_nc_fid.createVariable('zw', 'double',\
                                   ('zw'))
w_nc_dim = w_nc_fid.createVariable('zt', 'double',\
                                   ('zt'))
# You can do this step yourself but someone else did the work for us.
#for ncattr in nc_fid.variables['time'].ncattrs():
#    w_nc_dim.setncattr(ncattr, nc_fid.variables['time'].getncattr(ncattr))
...
#                    'var_desc': u"Air temperature",\
#                    'statistic': u'Mean\nM'})
w_nc_fid.variables['zw'][:] = numpy.abs(zf[1:12])
w_nc_fid.variables['zt'][:] = numpy.abs(zc[1:11])
w_nc_fid.close()  # close the new file
